...
setMethod("PnodeMargin",c("NeticaNode"),
          function(node,net=NULL) NodeBeliefs(node))
setMethod("PnodeEAP",c("NeticaNode"),
          function(node,net=NULL) c(NodeExpectedValue(node))) #clear SD attribute c()
setMethod("PnodeSD",c("NeticaNode"),
          function(node,net=NULL) {
            ev <- NodeExpectedValue(node)
            if (is.na(ev)) return(NA)
            attr(ev,"std_dev")
            })
setMethod("PnodeMedian",c("NeticaNode"),
          function(node,net=NULL)
            names(which(cumsum(NodeBeliefs(node))>=.5))[1])
setMethod("PnodeMode",c("NeticaNode"),
          function(node,net=NULL)
